...
export function ProfileView({
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (photoUrl && photoUrl !== social?.profile.foto_perfil_url) {
        onUpdateProfile({ foto_perfil_url: photoUrl });
      }
    }, 1000);

    return () => clearTimeout(timeout);
  }, [photoUrl, social?.profile.foto_perfil_url, onUpdateProfile]);
  ...
  return (
    <section className="entity-view">
      <ViewHero 
        ...
        variant="perfil"
        profileImage={social?.profile.foto_perfil_url}
      />
